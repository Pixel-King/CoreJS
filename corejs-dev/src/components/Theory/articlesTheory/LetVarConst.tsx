...
const LetVarConst: React.FC =() => {
    ...
    <p><code>let</code> и <code>const</code> ведут себя одинаково по отношению к лексическому окружению, области видимости.</p>
    <p>Но <code>var</code> - это совершенно другой зверь, берущий своё начало с давних времён. Обычно <code>var</code> не используется в современных скриптах, но всё ещё может скрываться в старых.</p>
    <p>На первый взгляд, поведение <code>var</code> похоже на <code>let</code>. Например, объявление переменной:</p>
    ...
    <p>Аналогично для циклов: <code>var</code> не может быть блочной или локальной внутри цикла:</p>
    <pre  className="text-bg-dark px-3 py-3">
        <code>
    ...
    <p>Как мы видим, <code>var</code> выходит за пределы блоков <code>if</code>, <code>for</code> и подобных. Это происходит потому, что на заре развития JavaScript блоки кода не имели лексического окружения. Поэтому можно сказать, что <code>var</code> - это пережиток прошлого.</p>
    <h2>&quot;var&quot; допускает повторное объявление</h2>
    <p>Если в блоке кода дважды объявить одну и ту же переменную <code>let</code>, будет ошибка:</p>
    <pre className="text-bg-dark px-3 py-3">
        <code>
            let user; <br />
    ...
    <p>Используя <code>var</code>, можно переобъявлять переменную сколько угодно раз. Повторные <code>var</code> игнорируются:</p>
    <pre className="text-bg-dark px-3 py-3">
        <code>
            var user = &quot;Pete&quot;; <br />
    ...
    <p>Объявления переменных <code>var</code> обрабатываются в начале выполнения функции (или запуска скрипта, если переменная является глобальной).</p>
    <p>Другими словами, переменные <code>var</code> считаются объявленными с самого начала исполнения функции вне зависимости от того, в каком месте функции реально находятся их объявления (при условии, что они не находятся во вложенной функции).</p>
    <p>Т.е. этот код:</p>
    <pre className="text-bg-dark px-3 py-3">
        <code>
            {`function sayHi() {`} <br />
    ...
    <p>...Технически полностью эквивалентен следующему (объявление переменной <code>var phrase</code> перемещено в начало функции):</p>
    <pre className="text-bg-dark px-3 py-3">
        <code>
            {`function sayHi() {`} <br />
            {`    *!*`} <br />
    ...
    <p>В примере выше <code>if (false)</code> условие никогда не выполнится. Но это никаким образом не препятствует созданию переменной <code>var phrase</code>, которая находится внутри него, поскольку объявления <code>var</code> &quot;всплывают&quot; в начало функции. Т.е. в момент присвоения значения <code>(*)</code> переменная уже существует.</p>
    ...
    <p>Поскольку все объявления переменных <code>var</code> обрабатываются в начале функции, мы можем ссылаться на них в любом месте. Однако, переменные имеют значение <code>undefined</code> до строки с присвоением значения.</p>
    <p>В обоих примерах выше вызов <code>alert</code> происходил без ошибки, потому что переменная <code>phrase</code> уже существовала. Но её значение ещё не было присвоено, поэтому мы получали <code>undefined</code>.</p>
    <h2>Итого</h2>
    <p>Существует 2 основных отличия <code>var</code> от <code>let/const</code>:</p>
    <ol>
    ...
    <p>Эти особенности, как правило, не очень хорошо влияют на код. Блочная область видимости - это удобно. Поэтому много лет назад <code>let</code> и <code>const</code> были введены в стандарт и сейчас являются основным способом объявления переменных.</p>
