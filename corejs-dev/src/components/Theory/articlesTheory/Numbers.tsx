...
const Numbers: React.FC =() => {
    return (
        ...
        <ol>
            ...
            <li><code>BigInt</code> числа дают возможность работать с целыми числами произвольной длины. Они нужны достаточно редко и используются в случаях, когда необходимо работать со значениями более чем <code>2<sup>53</sup></code> или менее чем <code>-2<sup>53</sup></code>.</li>
        </ol>
        ...
        <ul>
            <li><p><strong>base=16</strong> — для шестнадцатеричного представления цвета, кодировки символов и т.д., цифры могут быть <code>0..9</code> или <code>A..F</code>.</p>
            </li>
            <li><p><strong>base=2</strong> — обычно используется для отладки побитовых операций, цифры <code>0</code> или <code>1</code>.</p>
            </li>
            ...
            <pre>
                <code className="text-bg-dark px-3 py-3">
        ...
        : Округление в меньшую сторону: <code>3.1</code> становится <code>3</code>, а <code>-1.1</code> — <code>-2</code>.</p>
        <p><code>Math.ceil</code>
        : Округление в большую сторону: <code>3.1</code> становится <code>4</code>, а <code>-1.1</code> — <code>-1</code>.</p>
        <p><code>Math.round</code>
        : Округление до ближайшего целого: <code>3.1</code> становится <code>3</code>, <code>3.6</code> — <code>4</code>, а <code>-1.1</code> — <code>-1</code>.</p>
        <p><code>Math.trunc</code> (не поддерживается в Internet Explorer)
        : Производит удаление дробной части без округления: <code>3.1</code> становится <code>3</code>, а <code>-1.1</code> — <code>-1</code>.</p>
        <p>Ниже представлена таблица с различиями между функциями округления:</p>
        <table className="table table-bordered text-center">
            <thead>
                <tr>
                    <th scope="col"></th>
                    <th scope="col"><code>Math.floor</code></th>
                    <th scope="col"><code>Math.ceil</code></th>
                    <th scope="col"><code>Math.round</code></th>
                    <th scope="col"><code>Math.trunc</code></th>
                </tr>
            </thead>
            <tbody>
                <tr>
        ...
        <ul>
            <li><p><code>isNaN(value)</code> преобразует значение в число и проверяет является ли оно <code>NaN</code>:</p>
            <pre className="text-bg-dark px-3 py-3">
                <code>
        ...
        <p>Существует специальный метод <a href="mdn:js/Object/is">Object.is</a>, который сравнивает значения примерно как <code>===</code>, но более надёжен в двух особых ситуациях:</p>
        <ol>
        ...
        <p>&quot;Второй аргумент <code>parseInt(str, radix)</code>&quot;
        Функция <code>parseInt()</code> имеет необязательный второй параметр. Он определяет систему счисления, таким образом <code>parseInt</code> может также читать строки с шестнадцатеричными числами, двоичными числами и т.д.:</p>
        <pre className="text-bg-dark px-3 py-3">
            <code>
        ...
        <p><code>Math.random()</code> возвращает псевдослучайное число в диапазоне от 0 (включительно) до 1 (но не включая 1)</p>
        <pre className="text-bg-dark px-3 py-3">
            <code>
        ...
        <p><code>Math.max(a, b, c...) / Math.min(a, b, c...)</code> возвращает наибольшее/наименьшее число из перечисленных аргументов.</p>
        <pre className="text-bg-dark px-3 py-3">
            <code>
        ...
        <p><code>Math.pow(n, power)</code> возвращает число <code>n</code>, возведённое в степень `power`</p>
        <pre className="text-bg-dark px-3 py-3">
            <code>
        ...
        <p>В объекте <code>Math</code> есть множество функций и констант, включая тригонометрические функции, подробнее можно ознакомиться в документации по объекту [Math](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Math).</p>

        <h2>Итого</h2>

        <p>Чтобы писать числа с большим количеством нулей:</p>

        <p>- Используйте краткую форму записи чисел - <code>&quot;e&quot;</code>, с указанным количеством нулей. Например: <code>123e6</code> это <code>123</code> с 6-ю нулями <code>123000000</code>.</p>
        <p>- Отрицательное число после <code>&quot;e&quot;</code> приводит к делению числа на 1 с указанным количеством нулей. Например: <code>123e-6</code> это <code>0.000123</code> (<code>123</code> миллионных).</p>

        <p>Для других систем счисления:</p>

        <p>- Можно записывать числа сразу в шестнадцатеричной (<code>0x</code>), восьмеричной (<code>0o</code>) и бинарной (<code>0b</code>) системах счисления</p>
        <p>- <code>parseInt(str, base)</code> преобразует строку в целое число в соответствии с указанной системой счисления: <code>2 ≤ base ≤ 36</code>.</p>
        <p>- <code>num.toString(base)</code> представляет число в строковом виде в указанной системе счисления <code>base</code>.</p>

        <p>Для преобразования значений типа <code>12pt</code> и <code>100px</code> в число:</p>

        <p>- Используйте <code>parseInt/parseFloat</code> для &quot;мягкого&quot; преобразования строки в число, данные функции по порядку считывают число из строки до тех пор пока не возникнет ошибка.</p>

        <p>Для дробей:</p>

        <p>- Используйте округления <code>Math.floor</code>, <code>Math.ceil</code>, <code>Math.trunc</code>, <code>Math.round</code> или <code>num.toFixed(precision)</code>.</p>
