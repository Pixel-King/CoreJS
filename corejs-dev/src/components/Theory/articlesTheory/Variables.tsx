...
const Variables: React.FC =() => {
    ...
    <p>Ключевое слово <code>var</code> - <em>почти</em> то же самое, что и <code>let</code>. Оно объявляет переменную, но немного по-другому, &quot;устаревшим&quot; способом.</p>
    ...
    <ul>
        ...
        <li>
            <p>Объявления (инициализация) переменных <code>var</code> производится в начале исполнения функции (или скрипта для глобальных переменных).</p>
        </li>
    </ul>
    ...
    <p>Если имя содержит несколько слов, обычно используется <a href="https://ru.wikipedia.org/wiki/CamelCase">верблюжья нотация</a>, то есть, слова следуют одно за другим, где каждое следующее слово начинается с заглавной буквы: <code>myVeryLongName</code>.</p>
    ...
    <h3>Создание переменной без использования <code>use strict</code></h3>
    ...
    <p>- <code>COLOR_ORANGE</code> гораздо легче запомнить, чем <code>&quot;#FF7F00&quot;</code>.</p>
    <p>- Гораздо легче допустить ошибку при вводе <code>&quot;#FF7F00&quot;</code>, чем при вводе <code>COLOR_ORANGE</code>.</p>
    <p>- При чтении кода <code>COLOR_ORANGE</code> намного понятнее, чем <code>#FF7F00</code>.</p>
    ...
    <p>- Используйте легко читаемые имена, такие как <code>userName</code> или <code>shoppingCart</code>.</p>
    <p>- Избегайте использования аббревиатур или коротких имён, таких как <code>a</code>, <code>b</code>, <code>c</code>, за исключением тех случаев, когда вы точно знаете, что так нужно.</p>
    <p>- Делайте имена максимально описательными и лаконичными. Примеры плохих имён: <code>data</code> и <code>value</code>. Такие имена ничего не говорят. Их можно использовать только в том случае, если из контекста кода очевидно, какие данные хранит переменная.</p>
    <p>- Договоритесь с вашей командой об используемых терминах. Если посетитель сайта называется &quot;user&quot;, тогда мы должны называть связанные с ним переменные <code>currentUser</code> или <code>newUser</code>, а не, к примеру, <code>currentVisitor</code> или <code>newManInTown</code>.</p>
    ...
    <p>- <code>let</code> - это современный способ объявления.</p>
    <p>- <code>var</code> - это устаревший способ объявления.</p>
    <p>- <code>const</code> - похоже на <code>let</code>, но значение переменной не может изменяться.</p>
